...
def download_tdc():
    ...
    for admet in admet_groups:
        ...
        train['smiles'].to_csv(f'./datasets/admet_group/{admet}/trainset_smiles.csv',index=False)
        train['labels'].to_csv(f'./datasets/admet_group/{admet}/trainset_labels.csv',index=False)
        
        test = pd.read_csv(f'./datasets/admet_group/{admet}/test.csv')    
        test = test.rename(columns={'Drug':'smiles', 'Y': 'labels'})
        test['smiles'].to_csv(f'./datasets/admet_group/{admet}/testset_smiles.csv',index=False)
        test['labels'].to_csv(f'./datasets/admet_group/{admet}/testset_labels.csv',index=False)
